...
- JSON 备份导出与导入恢复
- 可选 Google Drive 云备份（写入用户自己的云盘空间）

隐私与数据：

- 正文和主要阅读数据默认保存在本地浏览器存储
- 不上传用户 TXT 正文到开发者服务器
- 云备份启用后，备份写入用户自己的 Google Drive 空间（appDataFolder），不占用开发者存储
- 权限用于阅读数据保存与云备份授权（storage / identity）
...
- Optional Google Drive backup stored in the user's own Drive space

Privacy and data handling:

- Main content and reading data stay in local browser storage
- No TXT body upload to developer servers
- If cloud backup is enabled, backups are stored in the user's own Google Drive (appDataFolder), not developer storage
- Permissions are used for local data and cloud authorization (storage / identity)
...
- [ ] 权限理由已填写（`storage` 用于阅读数据保存，`identity` 用于 Google Drive 云备份授权）
- [ ] 审核通过后状态变为 "Published"
