...
- 形态：Manifest V3 扩展
- 平台：Chrome / Edge
- 数据策略：正文保存在 IndexedDB，轻量设置与进度可镜像到浏览器账号同步（Chrome/Edge `storage.sync`）
...
- 左右方向键切换章节，空格键翻页（边界自动跨章节）
- 书签、最近阅读列表、阅读统计面板
- 快捷键帮助浮层
- JSON 备份导出 / 导入恢复
- 基于浏览器账号同步（Chrome/Edge `storage.sync`）的轻量设置与阅读进度同步
