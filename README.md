# Xyfy.TxtReader

Chrome 和 Edge 可用的 TXT 小说阅读扩展，面向本地小说阅读场景，重点提供书本式双页、单页滚动、沉浸阅读和本地数据迁移能力。

## 插件信息

- 名称：Xyfy TXT Reader
- 版本：0.2.0
- 形态：Manifest V3 扩展
- 平台：Chrome / Edge
- 数据策略：正文保存在 IndexedDB，轻量设置与进度可镜像到 `chrome.storage.sync`

## 当前功能

- 本地 TXT 导入，自动识别章节目录
- DOM 实测分页，双页书本模式与自适应单页降级
- 单页滚动模式，支持整屏滚动与小步滚动
- 页面内沉浸模式，隐藏侧栏与控制区专注阅读
- 左右方向键统一翻页，在边界继续按会切换章节
- 书签、最近阅读列表、阅读统计面板
- 快捷键帮助浮层
- JSON 备份导出 / 导入恢复
- `chrome.storage.sync` 轻量同步设置与阅读进度
- Google Drive 云备份（连接、上传、列表、恢复、删除）
- 大章节增量分页、章节分页缓存、空闲时后台预热

## 本地运行

1. 打开 Chrome 或 Edge 的扩展管理页面。
2. 启用开发者模式。
3. 选择“加载已解压的扩展程序”。
4. 选择当前项目目录。
5. 点击扩展图标，打开阅读器。

### Google Drive 云备份配置（开发态）

1. 在 Google Cloud Console 创建 OAuth Client（Chrome Extension）。
2. 将 `manifest.json` 中 `oauth2.client_id` 替换为你的真实 Client ID。
3. 保留当前 scope：`https://www.googleapis.com/auth/drive.appdata`。
4. 重新加载扩展后，在阅读器侧栏点击“连接云服务”完成授权。

说明：

- 云备份仅使用 Drive 的 `appDataFolder`，不会将备份暴露到用户普通云盘目录。
- 云备份文件写入用户自己的 Google Drive（`appDataFolder`），占用的是用户云盘空间，不占用开发者存储。
- 若当前浏览器环境不支持 `chrome.identity`，会自动回退到本地备份流程。

## 打包发布

### 一键打包

1. 安装依赖（如需）：`npm install`
2. 执行测试：`npm test`
3. 生成发布包：`npm run build:zip`

执行后会在 `dist` 目录下生成：

- `xyfy-txt-reader-v<version>.zip`

说明：

- ZIP 根目录就是扩展根目录（`manifest.json` 在压缩包根层级）
- 已自动排除 `tests`、`docs`、`.git`、`node_modules`、`dist`

### 发布前检查清单

- `manifest.json` 里的 `version` 已递增
- 图标与商店截图已准备（建议至少 128x128 图标）
- 权限最小化（当前仅 `storage`）
- 权限最小化（当前为 `storage` + `identity`，并访问 `https://www.googleapis.com/*`）
- 已验证升级安装后的阅读进度与设置兼容性
- 隐私政策链接已准备：`https://xyfy.github.io/Xyfy.TxtReader/privacy-policy.html`
- 商店文案素材可直接使用：`docs/store-listing.md`

### GitHub Pages 隐私政策链接启用

1. 推送当前仓库到 GitHub 默认分支（例如 `main`）。
2. 进入仓库 Settings -> Pages。
3. Source 选择 `Deploy from a branch`。
4. Branch 选择 `main`，目录选择 `/ (root)`，保存。
5. 等待 Pages 发布完成后，使用：`https://xyfy.github.io/Xyfy.TxtReader/privacy-policy.html`

### Chrome Web Store 发布

1. 进入 Chrome Web Store Developer Dashboard。
2. 新建扩展并上传 `dist` 下 ZIP。
3. 完成商店信息：描述、截图、分类、隐私政策。
4. 提交审核并发布。
5. 后续更新重复以上流程，仅需先提升版本号。

### Edge Add-ons 发布

1. 进入 Edge Add-ons 开发者中心。
2. 创建扩展并上传同一 ZIP。
3. 补充商店信息并提交审核。

### Firefox AMO 发布（可选）

1. 进入 AMO 开发者后台上传 ZIP。
2. 检查 Manifest V3 API 兼容性并按提示修复。
3. 完成审核后发布。

## 常用快捷键

- `← / →`：翻页；在边界继续按时切换章节
- `Space`：书本模式切下一章；滚动模式下滚一整屏
- `Shift + Space`：滚动模式上滚一整屏或回上一章
- `↑ / ↓`：单页滚动模式下小步滚动
- `B`：添加书签
- `Tab`：显示 / 隐藏左侧栏
- `H` 或 `?`：打开快捷键帮助浮层
- `Alt + D`：切换分页调试面板
- `Esc`：关闭快捷键帮助浮层

## 当前目录

- popup: 扩展弹窗
- reader: 阅读器页面与交互逻辑
- background: MV3 service worker
- modules: 核心模块（解析、分页、备份、存储）
- tests: 分页、备份、同步、云备份抽象层测试
- docs: 规划文档
